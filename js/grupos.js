...
//eventos 
const cerrarAlumno=document.querySelector('#cerrarAlumno').addEventListener('click',ocultarSeccionAlumno);
const aGrupo=document.querySelector('#addGrupo').addEventListener('click',obtenerMaterias);
//tablas 
    ...
    function Eliminar_Grupo(id, objetivo){
        const elementoEliminar = document.querySelector("#aceptar");
        ...
        if(elementoEliminar){
            elementoEliminar.addEventListener('click', (e) => {
                e.preventDefault();
                console.log('aqui');
            ...
            if(data.length === 0){
                ...
                data.data.forEach(info =>{
                    html+=`<tr>
                            ...
                            <td><button class="eliminar cerrarAlumno" data-id="${info.Matricula_Alumno}" on>&Chi;</button></td>`;
                });
            }
    ...
    function Eliminar_AlumnoGrupo (id,matricula, objetivo) {
        const  elementoAlumnoGrupo = document.querySelector(`${objetivo}`);
        
        elementoAlumnoGrupo.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopImmediatePropagation();
...
//funcion a la espera del submit del usuario
    function Agregar_Grupo(objetivo){
        const elementoAGrupo= document.querySelector(`${objetivo}`);

        elementoAGrupo.addEventListener('click', e =>{
            e.preventDefault();
            e.stopImmediatePropagation();
            if(e.target.classList.contains('aceptar')){
